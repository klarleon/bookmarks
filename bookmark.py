...
"""
class BookmarksListManager (object):
	def __init__(self, bookmarks=None):
		...
		if bookmarks:
			...
			self.load_file()
		self.tableview = BookmarksListTableView(self)
	...
	def get_title(self, index):
		# Gets the item in the list at the specific index and returns the displayed bookmark title.
		# If bookmark.title is an empty string, set it to the filename
		title = self.bookmarks[index]['title']
		if title == '':
			title = self.get_path(index).split('/')[-1]
		return title
	...
	def set_title(self, index, title):
		# Changes the displayed title of the bookmark at the specified index.
...
class BookmarksListTableView (ui.View):
	def __init__(self, bm_manager):
		self.name = 'Bookmarks'
...
test_bookmarks = [{'title': 'Bookmark Source Code', 'path': os.path.expanduser('~/Documents/My Scripts/bookmarks/bookmark.py')},
		{'title': '', 'path': os.path.expanduser('~/Documents/Tools/IstaFlow/istaflow.py')}]
		
		
def main():
	# TODO Load any settings first
	bm = BookmarksListManager()
